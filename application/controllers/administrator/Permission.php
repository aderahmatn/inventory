		...
		if ($this->form_validation->run()) {
			...
			if ($save_permission) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->response['message'] = 'Your data has been successfully stored into the database. '.anchor('administrator/permission/edit/' . $save_permission, 'Edit Permission').' or  '.anchor('administrator/permission', ' Go back to list');
				} else {
					set_message('Your data has been successfully stored into the database. '.anchor('administrator/permission/edit/' . $save_permission, 'Edit Permission'));
	        		$this->response['success'] = true;
	...
	public function edit_save($id)
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($save_permission) {
				if ($this->input->post('save_type') == 'stay') {
					$this->response['success'] = true;
					$this->response['message'] = 'Your data has been successfully updated into the database. '.anchor('administrator/permission', ' Go back to list');
				} else {
					set_message('Your data has been successfully updated into the database. ');
	        		$this->response['success'] = true;
					$this->response['redirect'] = site_url('administrator/permission');
				}
			} else {
