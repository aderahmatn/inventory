...
class Rest extends Admin	
{
	...
	public function save_rest()
	{
		if ($this->form_validation->run()) {
			...
			if ($this->input->post('save_type') == 'stay') {
				...
				$this->response['message'] = 'Your data has been successfully saved into the database. '.anchor('administrator/rest', ' Go back to list').' or '.anchor('administrator/rest/view/'.$id_rest, ' View');
			} else {
				set_message('Your data has been successfully saved into the database. '.anchor('administrator/rest/view/'.$id_rest, ' View'), 'success');
        		$this->response['success'] = true;
	...
	*/
	public function delete($id)
	{
		...
		if ($remove) {
            set_message('Rest has been deleted.', 'success');
		} else {
            set_message('Error delete rest.', 'error');
		}
